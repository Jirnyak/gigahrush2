...
for i in range(70, 80):
    ...
    if '### Детальный Workflow Реализации' in content:
        content = content.split('### Детальный Workflow Реализации')[0]
        
    if '## Ваши шаги:' in content:
        content = content.split('## Ваши шаги:')[0]
        
    task_data = tasks.get(i)
    if task_data:
        footer = common_footer.replace('<STUBS_PLACEHOLDER>', task_data['stubs']).replace('<INTEGRATION_PLACEHOLDER>', task_data['integration'])
        ...
        print(f"No task data for marx_{i}")

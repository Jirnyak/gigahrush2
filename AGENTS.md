...
> **ВЕТКА РАЗРАБОТКИ — `main`** (решение владельца 2026-09-13, «возвращение
> короля»). Работаем и коммитим ПРЯМО В НЕЁ. Ветка `torus`, бывшая линией
> разработки с августа, **снесена**: её дерево совпало с `main` на слиянии
> `832796e0`, уникальных коммитов не осталось. Ритуал «main принимает торовую
> линию» умер вместе с ней — принимать больше нечего и неоткуда. В репозитории
> живыми остаются `main`, `gh-pages` (развёртывание Pages) и
> `backup/origin-main-2026-08-06` (дореформенная линия); остальное снесено, с
> адресами — [markoaudit/branch-graveyard.md](markoaudit/branch-graveyard.md).
>
> Что из этого следует и о чём НЕ забыть:
> * Гейт теперь стоит ровно там же, где раньше, — в коммите. Отдельной ветки,
>   которая ловила бы работу до общей линии, больше НЕТ, поэтому «соберу и
>   померю потом» перестало быть безопасным: `ctest` зелёный и гейт в том же
>   коммите — единственная защита, которая осталась.
> * Прямой пуш в `main` доступен всем, у кого есть доступ к репозиторию. Защита
>   ветки (обязательный PR) НЕ включена — это настройка GitHub, и включить её
>   может только владелец. Прецедент, почему это важно: `bb62c64a` прилетел в
>   `main` от базы месячной давности, без единого гейта и с понижённым пином
>   (разбор — [markoaudit/plans/marko-vr-pick.md](markoaudit/plans/marko-vr-pick.md)).
> * Работа от базы старше недели требует перебазирования ДО пуша. Месяц
>   расхождения не сливается текстом — он переносится по смыслу.

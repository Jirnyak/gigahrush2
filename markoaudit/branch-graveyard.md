...
**ИСПОЛНЕНО.** Снесено 14 локальных веток и 10 рабочих деревьев, 7 веток на
`origin`. После уборки в репозитории осталось ТРИ ветки: `main`, `gh-pages`,
`backup/origin-main-2026-08-06` (плюс четыре dependabot-PR). `ctest 8/8` на
`main` после уборки — сборка не заметила.

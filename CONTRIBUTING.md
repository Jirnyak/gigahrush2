...
Thank you for your interest in contributing to **Jirnyak/gigahrush2**. This project operates under strict technical standards: deep mathematical and domain correctness, zero-slop code, explicit typing, and zero unverified assumptions.
...
1. **Universal Carve Formula**:  Subtraction of voxel volumes must follow exact CSG boolean geometry without manifold cracks.
2. **6-Connected BFS Support Graph**:  Floating voxel clusters with zero ground-path connection must immediately collapse under gravity.
3. **Seismic Sound Synthesis**:  Sub-bass tectonic rumble must be dynamically synthesized from mass-collapse momentum equations.
4. **Chunk Mesh Streaming**:  Toroidal world partitions must stream in O(1) time without blocking WebGL presentation.
...
### 2.1 Prerequisites
* **Tech Stack**: `TypeScript 5.8 / WebGL 2.0 / Web Audio API / Universal Carve System / BFS Support Graph`
...
2. **Regression Verification**: Ensure all existing test suites pass cleanly with `npm test && npm run build`.

...
> **Уточнение владельца, 2026-08-12 — читать как часть принципа, а не как
> примечание.** Ограничено число СИСТЕМ, а не число ВХОДОВ. У формулы нет и не
> должно быть предела на количество контекста, который она читает: номер этажа,
> вид задачи, что именно надо сделать, кто выдал, сколько штук, есть ли срок —
> всё это может входить в одну величину одновременно, и это правильно. Плохо
> не «много слагаемых», плохо «новая подсистема ради слагаемого».
>
> Отсюда два практических следствия, и они операционные:
>
> 1. **Выбор «взять A ИЛИ B» — почти всегда ложная развилка.** Если и A, и B уже
>    существуют в дереве, правильный ответ обычно «оба, слагаемыми». Вопрос
>    «откуда брать сложность квеста — из глубины или из вида задачи?» поставлен
>    неверно по построению.
> 2. **Расширяемость — это форма кода, а не обещание.** Величина, собранная из
>    контекста, пишется так, чтобы добавление ЕЩЁ ОДНОГО входа было одной
>    строкой в одной функции, и чтобы каждое слагаемое называло, из какой
>    существующей системы оно взято. Функция, куда нельзя дописать строку, не
>    выполняет этот принцип, сколько бы входов в ней ни было сегодня.
>
> Проверка на нарушение простая: **добавляется ли новая таблица, новая колонка
> или новый счётчик?** Если нет — количество входов не является поводом для
> спора. Если да — вот это и надо обсуждать.
...
5. **RPG и бой.** 8 атрибутов (степень двойки для хранения): сила (+1%
   ...
   **Урон рушит мир**: тип урона × материал = вероятность сноса субвокселя
   ...
   очередь из автомата лишь царапает краску). **Инвентарь** 8×8 + вес (64 кг
   база + 4 кг за очко Силы) + слоты оружие/броня/пси/инструменты
   (заряд/прочность — фонарик).
...
| Вес (64 кг + 4 кг за Силу) | `mass_g` во всех 442 строках items.csv; `inventory_mass_g` + `carry_capacity_g` ([rpg.h]); последствия в [encumbrance.h] | **ЕСТЬ (2026-08-06)**: та же колонка `mass_g` в целых граммах, что у пропов и мобов — одна единица на дерево. Ноша = 64 + 4·Сила (не Выносливость: её в `Attr` нет). Перегруз бьёт четырьмя путями, два из которых НЕ новые правила — `Mass` = тело + ноша, поэтому падение (`E=mv²/2`) и отбрасывание (`p=mv`) увидели рюкзак через законы, которые не менялись |
| Нужды еда/вода/туалет/сон | `Needs{food,water,sleep,pee,poo}` + needs_step | **ЕСТЬ (2026-08-06)**: тик у ВСЕХ воплощённых на живом этаже, O(n) по этажу; жители рождаются в своей точке суток (`needs_roll_resident`). Вместе с зональными полями комнат ([room_zone.h]) и восстановлением в комнате это закрыло [problems.md] §27: на живом прогоне `own_ai` не 0, гистограмма интентов ДВИЖЕТСЯ (`drink` 78 → 39), `crowd_dead_total=0`. Холодные 950k по-прежнему тикает только макросим |
...
| Лифты 4×4 fast + 4×4↓ + 4×4↑ столбами | 16 шахт (4×4), каждая предлагает все три перехода через меню (`L`) | **РЕШЕНО владельцем 2026-08-12: 16 шахт + роль в меню.** Манифест просил 48 столбов = три набора по 4×4; принято, что три вида ПЕРЕХОДА не требуют трёх наборов КОЛОНН. Решение к тому же единственное, которое не двигает `kLatticeDim`: `nav::kNodes == kLatticeCount == kLatticeDim³`, поэтому 5 узлов на ось увели бы тонкий нав-бейк со 128 МиБ на 250 и сломали пин формата нав-кэша (`kNavCoarseWire == 13056` при 64 узлах). См. [elevators.md](elevators.md) |
...
| Баллистика снарядов | `projectile_step` + carve от попаданий; **снаряд не помнит владельца** | **СОВПАДАЕТ с 2026-08-12** ✓ `Projectile::team` удалено, все три исключения и фракционный фильтр сняты, три ветки сведены в один проход по ближайшей цели. `source` остался только как АТРИБУЦИЯ убийства для XP. Одно из «исключений» оказалось дефектом геометрии: снаряд рождался внутри собственной сферы попадания стрелка — починено стволом на `kMuzzleForward`. [problems.md] §33. **ЗАКРЫТО ПОЛНОСТЬЮ 2026-08-13**: гранаты построены — `ProjType::Grenade` отражается от ГРАНИ клетки (не от буквы оси), детонирует по фитилю (`ttlMs`), бьёт площадью через `apply_damage` и рвёт геометрию через существующую `CarveProposalQueue`. В проходе детонации НЕТ владельца: бросивший попадает под свой взрыв (`[gren] OWN BLAST hp 100 -> 28`). В тот же день удалено ПОСЛЕДНЕЕ исключение — `p.source != victim` в контактном пути: оно страховало 4-миллиметровый зазор между `kMuzzleForward` и ростом стрелка, теперь зазор гарантирован `muzzle_point` (выход луча из сферы попадания). [problems.md] §50. **Остаток:** гранат не бросают монстры (ни одна строка `data/mobs.csv` не выбирает `grenade`), и взрыв не проверяет ПРЕГРАДУ — стена не укрывает от осколков |

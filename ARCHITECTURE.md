...
| Баллистика снарядов | `projectile_step` + carve от попаданий; **снаряд не помнит владельца** | **СОВПАДАЕТ с 2026-08-12** ✓ `Projectile::team` удалено, все три исключения и фракционный фильтр сняты, три ветки сведены в один проход по ближайшей цели. `source` остался только как АТРИБУЦИЯ убийства для XP. Одно из «исключений» оказалось дефектом геометрии: снаряд рождался внутри собственной сферы попадания стрелка — починено стволом на `kMuzzleForward`. [problems.md] §33. **Остаётся GAP:** гранат нет вовсе, все 19 не-`Normal` стволов отложены ([ranged_table.h:20-27]), поэтому «граната скачет по вокселям, осколки бьют и владельца» проверено пока только на пулях |

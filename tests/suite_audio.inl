...
static void test_dsp_math_primitives() {
    ...
    bool prngInRange = true;  // один CHECK на цикл: 10k ассертов ломали ратчет
    for (int i = 0; i < kPrngSamples; ++i) {
        float val = splitmix32_f01(state);
        prngInRange = prngInRange && val >= 0.0f && val < 1.0f;
        sum += val;
    }
    CHECK(prngInRange);
...
static void test_geiger_poisson_synth() {
    ...
    bool geigerSane = true;  // один CHECK: 48k ассертов на буфер ломали ратчет
    for (float sample : buf)
        geigerSane = geigerSane && !std::isnan(sample) && !std::isinf(sample) &&
                     sample >= -2.5f && sample <= 2.5f;
    CHECK(geigerSane);
}
...
static void test_siren_c40_synth() {
    ...
    bool sirenSane = true;
        for (float s : buf) {
            sirenSane = sirenSane && !std::isnan(s) && !std::isinf(s) &&
                        s >= -1.5f && s <= 1.5f;
        }
    CHECK(sirenSane);
    }
...
static void test_audio_mixer_polyphony_and_limiting() {
    ...
    bool mixSane = true;
        for (int i = 0; i < kAudioBlockFrames * 2; ++i) {
            ...
            mixSane = mixSane && !std::isnan(sample) && !std::isinf(sample);
            // Master bus limiter strictly guarantees [-1.0, 1.0] output range
            mixSane = mixSane && sample >= -1.0f && sample <= 1.0f;
        }
    CHECK(mixSane);
    }
}

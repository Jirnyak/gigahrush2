// suite_gravity_regimes.inl — spec2.txt §5.1
//
// GATE: All 8 GravityRegimes tested for fall direction, fluid anisotropy, and
// carve. A failure here pinpoints a hardcoded -Z axis.
//
// The spec mandates:
...
//  - Verified via: rg 'downAxis' src/sim/fluid.cpp => empty
...
// --- §5.1 d: GATE — verify 'downAxis' literal is absent from fluid.cpp -----
// This is a compile-time check: the string search is done in tests/ because
// the rg command is a dev tool. Instead we verify isotropy properties that
// only hold if regime_down() is used (see test_fluid_isotropy above).
// The structural test is: test_fluid_falls_in_regime_direction passes for ALL
// 6 directional regimes simultaneously — impossible with a single hardcoded axis.

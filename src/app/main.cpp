...
int main(int argc, char** argv) {
    ...
    //
    // БЕЗ БУФЕРИЗАЦИИ (_IONBF), и это исправление, а не выбор из вкуса.
    // Сначала здесь стояло _IOLBF («строчная»), и первый же прогон владельца
    // под виндой дал ПУСТОЙ файл. Две причины сразу, обе — MSVC:
    //   1. его CRT не поддерживает _IOLBF и молча считает её ПОЛНОЙ
    //      буферизацией (документировано);
    //   2. для буферизованных режимов size=0 недопустим, так что вызов ещё и
    //      проваливался целиком, оставляя поток таким, каким его сделал
    //      freopen, — то есть полностью буферизованным.
    // Дальше достаточно было закрыть не то окно (у .bat их два — консоль и
    // игра), чтобы процесс умер вместе с непролитым буфером. Профиль снимают
    // именно тогда, когда игра тормозит, а такие сессии кончаются Диспетчером
    // задач — прибор обязан переживать убийство процесса. _IONBF это даёт:
    // каждая строка уходит в файл сразу. Цена — писать чаще, и она заплачена
    // осознанно: инструмент, который теряет данные, не инструмент.
    if (profWanted) {

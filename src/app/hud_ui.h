...
// Атмосфера клетки под телом — app снимает её с газового пасса и передаёт
// ЧИСЛАМИ, чтобы худ не включал gpu_gas_pass.h (рендер — не источник истины,
// но газ и живёт на GPU; читатель у него уже есть, мы второй).
struct HudGas {
    std::uint8_t tox = 0, smoke = 0, oxy = 255, heat = 0;
    bool valid = false;
};

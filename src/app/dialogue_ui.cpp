...
void draw_dialogue_window_ui(DialogueSession& session, DialogueAction& outAction) {
    ...
    ImGui::SetNextWindowSize(ImVec2(780.0f, 320.0f), ImGuiCond_Always);

    ImGui::Begin("ДИАЛОГ С ВЫЖИВШИМ / DIALOGUE##dialogue_window", nullptr,
                 ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove |
                 ImGuiWindowFlags_NoSavedSettings);

    // 1. Header: Speaker Badge, Faction, Situation, Vitality
    ...
    ImGui::SameLine();
    ImGui::TextColored(ImVec4(0.80f, 0.80f, 0.67f, 1.0f), "| Состояние: [%s]", sitName);
    ...
    ImGui::ProgressBar(hpFrac, ImVec2(160.0f, 18.0f), hpBuf);

    ImGui::Separator();
    ImGui::Spacing();

    // 2. Main Speech Body & Rumours
    ImGui::BeginChild("##dialogue_speech_scroll", ImVec2(0.0f, 150.0f), true);
    ...
    if (ImGui::Button("[1] Расспросить (Слухи)", ImVec2(180.0f, 28.0f)) ||
    ...
    if (session.contractText[0]) {
        ...
        if (ImGui::Button("[2] Взять контракт", ImVec2(150.0f, 28.0f)) ||
    ...
    if (session.questText[0]) {
        ...
        if (ImGui::Button("[3] Взять задание", ImVec2(150.0f, 28.0f)) ||
    ...
    if (session.traderNear) {
        if (ImGui::Button("[4] Торговля", ImVec2(120.0f, 28.0f)) ||
    ...
    if (session.canPossess) {
        if (ImGui::Button("[P] Вселиться", ImVec2(120.0f, 28.0f)) ||
    ...
    ImGui::SetCursorPosX(ImGui::GetWindowWidth() - 160.0f);
    if (ImGui::Button("[E / Esc] Завершить", ImVec2(140.0f, 28.0f)) ||

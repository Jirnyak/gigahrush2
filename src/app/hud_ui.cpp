...
void hud_ui_draw(const HudContext& ctx) {
    // 3D-режим: стекло худа плоское и рисуется ОДИН раз поверх обеих картинок
    // — прицел повисает посреди шва, углы уезжают на чужой глаз. Пока худ не
    // живёт в мировом пространстве, честнее снять его целиком, чем показывать
    // сломанным. Окна ImGui (настройки, консоль) остаются: ими режим и
    // выключают, и они честно моно.
    if (ctx.stereo3d) return;

...
// (бамп/блики) заперта в слабом хвосте, доминанты честные. K = 8: замер
// полного полуреза дал 2× fps при заметной деградации ВСЕГО света; топ-8
// покрывает типичную плотность доминант (mean списка 10.6, доминант ~4-8).
// В шейдеры едет дефайном GIGA_LIGHT_FULLRES_K (CMake парсит эту строку).
static constexpr uint32_t kFullResLights = 8;

...
private:
    ...
    // ФРЕЙМ ПОСЛЕДНЕГО ПРОГОНА — для диагностики, и это не удобство, а закон
    // изотропии: «вниз» у нас одно из ВОСЬМИ значений GravityRegime, и ни одна
    // ось не особенная. Разбор живого набора «слоями z и колоннами xy» (как он
    // печатался до 2026-09-23) читается верно ровно при NegZ и врёт при всех
...
private:
    ...
    //
    // ДВА ЭТАЖА, а не один, и это лечение виндового замера. Раньше pack-пасс
    // писал ПРЯМО в host-visible буферы: на едином поле Apple это бесплатно,
    // а на дискретной карте host-visible без device-local — системная память
    // за PCIe, и шейдер долбил в неё по записи на клетку. Замер владельца
    // (RTX 3080, 2026-09-14): simphys 53.8 мс при рисовании мира в 1.2 мс,
    // кадр 101 мс, окно шва при этом ПОЛНОЕ (8415 живых > 8192 слотов) —
    // то есть 8 МиБ через шину каждый кадр, врассыпную.
    //
    // Теперь шейдер пишет в *Dev_ (настоящая VRAM), а на хост регион кадра
    // уезжает ОДНОЙ vkCmdCopyBuffer — пачкой DMA вместо тысяч разрозненных
    // записей. Хостовая половина просится КЭШИРУЕМОЙ
    // ([vk_buffer.h] create_host_readback): её читает процессор, а чтение из
    // write-combining памяти — вторая половина того же замера.
    VulkanBuffer pageDev_;  // цель pack-пасса: VRAM
    VulkanBuffer maskDev_;
    VulkanBuffer listDev_;
    VulkanBuffer pageBack_; // приёмник копии: хост, кэшируемый

...
public:
    ...
    // Изменённая маска клетки на шве GPU→CPU: XOR слов (старое ^ новое).
    // Сид судьи связности (S20.5 «сеять от изменения»: судятся опорные
    // соседи реально изменённых атомов, не вся клетка) и нав-долг (ci).
    struct ChangedMask {
        std::uint32_t ci;
        std::uint64_t bits[8];
    };
    ...
    // изменившейся маской + XOR изменённых битов (нав-долг и сид судьи
    // у вызывающего).
    void apply_readback(World& world, VoxelMirror& mirror,
                        std::vector<ChangedMask>* changedMasks = nullptr);

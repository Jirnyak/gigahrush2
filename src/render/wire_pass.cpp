...
struct SimPush {
    ...
    vec4 player; // xyz player pos, w push radius
};
...
bool WirePass::init(VulkanDevice* dev, VkRenderPass renderPass,
    ...
    if (vkAllocateDescriptorSets(dev_->device, &ai, &set_) != VK_SUCCESS)
        return false;
    VkDescriptorBufferInfo bi{points_.buffer, 0, VK_WHOLE_SIZE};
...
void WirePass::record_sim(VkCommandBuffer cmd, const vec3& playerPos, float dt) {
    ...
    p.player = vec4{playerPos.x, playerPos.y, playerPos.z, 0.9f};

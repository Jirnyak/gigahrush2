...
void ClothPass::record_sim(VkCommandBuffer cmd, float dt, vec3 gravity) {
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, simLayout_, 0,
                            1, &set_, 0, nullptr);
    SimPush p{};
    p.sim = vec4{dt, 0.0f, static_cast<float>(sheetCount_), 0.985f};

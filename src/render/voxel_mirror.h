...
public:
    ...
    // 4194304 -> 4194303 (2026-09-13): МИНУС ОДНА СТРАНИЦА, и это не косметика.
    // Пул биндится ЦЕЛИКОМ одним storage buffer (raymarch_pass.cpp: range =
    // kPoolBytes), а 4194304 x 1 КиБ = 4 294 967 296 = ровно 2^32 байта.
    // maxStorageBufferRange у NVIDIA/AMD/Intel = 4 294 967 295 — ровно на
    // ОДИН байт меньше. Значит на каждом десктопном драйвере привязка была
    // вне спеки, а чтение пула из шейдера — UB; MoltenVK на маке это
    // проглатывал, поэтому дефект восемь месяцев жил невидимым и всплыл
    // первым же виндовым билдом (стим-аудит B4, [markoaudit/plans/
    // steam-readiness.md]). Одна страница возвращает привязку в спеку
    // (4 294 966 272 байта) и НЕ трогает исходный вывод капа: он и так брался
    // с запасом «превышает даже все-клетки-смешанные» против измеренных ~690k
    // страниц — шесть порядков запаса не замечают вычета единицы.
    static constexpr std::uint32_t kPageCap = 4194303u;

...
// ATTR1: `attr str|agi|end|int|per` — spend one unspent point. The app drains the
...
bool cmd_attr(ConsoleContext& ctx, int argc, const char* const* argv,
    ...
    if (argc < 2 || !argv[1] || !argv[1][0]) {
        if (out && cap) std::snprintf(out, cap, "usage: attr <str|agi|end|int|per>");
        return false;
    }
    ...
    else if (std::strcmp(a, "end") == 0 || std::strcmp(a, "END") == 0)
        bit = ConsoleRequest::AttrEnd;
    else if (std::strcmp(a, "int") == 0 || std::strcmp(a, "INT") == 0)
        bit = ConsoleRequest::AttrInt;
    else if (std::strcmp(a, "per") == 0 || std::strcmp(a, "PER") == 0)
        bit = ConsoleRequest::AttrPer;
    else {
        if (out && cap) std::snprintf(out, cap, "attr: want str|agi|end|int|per, got %s", a);
        return false;
    }
...
std::uint32_t complete_attr(const ConsoleContext&,
    ...
    static constexpr const char* kOpts[] = {"str", "agi", "end", "int", "per"};
    std::uint32_t n = 0;

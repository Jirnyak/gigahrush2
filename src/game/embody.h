...
// ГОДНА ЛИ СУЩНОСТЬ ПОД ВСЕЛЕНИЕ — предикат обоих путей (смерть и
// добровольное), bugs.md Б6.1.
//
// Он существует, потому что оба отбора спрашивали ГОЛЫЙ СЛОТ
// (`pool.valid(id) && pool.alive(id)`), а переработка слотов ВЗВЕДЕНА. Щуп в
// живой игре владельца поймал ровно предсказанное:
//
//     EVENT death: вселение в запись 1287 | gen ref=0 pool=1 | corpse=1 rigid=1
//
// Поколение разъехалось — слот записи переиспользовали, а сущность-ТРУП
// осталась со старой ссылкой, и отбор счёл её живым жителем. Игрок получал
// сегменты рагдолла на `JointLink` по построению (симптом «на мне висит труп»)
// и `RigidBody`, из-за которого `physics_step` тело пропускает, а движением
// рулит солвер — то есть Б6.1 и половина Б6.2 одной сущностью.
//
// Честная проверка ссылки через время одна — `npc_ref_current` (S20.3, тут же
// выше: «сравнение поколений вручную — дефект»). Отбор её не звал.
// Исключение останков — вторая нога: слот может совпасть, а тело всё равно
// быть трупом.

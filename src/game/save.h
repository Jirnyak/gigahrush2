...
// v23 (2026-09-23, снос системы хазарда клеток): формат байт-в-байт ТОТ ЖЕ —
// бамп здесь потому, что поехал СЛОВАРЬ. Из `data/materials.csv` сняты три
// строки (`electric_grate`, `acid_pool`, `fire_cell`) вместе с двумя их
// рыхлыми двойниками, kMatCount 43 -> 38, и все id ниже 16-го сдвинулись.
// `CellType` лежит в снимке этажа МИЛЛИОНАМИ — и в типах клеток, и в
// страницах субвокселей. Старый файл прочитался бы без единой ошибки и
// отрисовал бы мир не теми материалами: стекло стало бы неоном, асфальт
// стеклом. Молчаливая порча вместо честного отказа — ровно то, против чего
// построен весь этот формат (раздел ниже). Этажи перегенерятся один раз.
inline constexpr std::uint32_t kSaveVersion = 23u;
...
// v5 -> v6 (2026-09-23): СЛОВАРЬ МАТЕРИАЛОВ, а не формат. Снос системы
// хазарда клеток снял три строки `data/materials.csv` и двух их рыхлых
// двойников, kMatCount 43 -> 38, id ниже 16-го сдвинулись на три. Этот файл
// несёт `CellType` миллионами — типы клеток И страницы субвокселей, — и
// старый снимок прочитался бы БЕЗ ЕДИНОЙ ОШИБКИ, просто нарисовав мир не
// теми материалами. Тихая порча вместо честного отказа — худший исход из
// возможных, поэтому бамп обязателен даже при неизменном байтовом формате.
// То же правило инвалидации (S20.6 закон 4): этаж перегенерируется.
inline constexpr std::uint32_t kFloorFileVersion = 6u;

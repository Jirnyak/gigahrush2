...
struct LightVisLamp {
    ...
    float radiusM = 0.0f;
    // Цвет лампы — вход взвешенного цвета кластера хвоста (см. ниже).
    vec3 color{1.0f, 1.0f, 1.0f};
};

// КЛАСТЕРИЗАЦИЯ ХВОСТА (заново 2026-08-24, light-cluster.md; первая попытка
// вырезана 86860f58 — синтез и паковка работали, но биннинг выбрасывал каждую
// ссылку гвардой id >= staticCount, механизм не светил ни кадра). Решения
// владельца, живые из плана и подтверждённые 2026-08-24: клетка держит топ-K
// ЧЕСТНЫХ ламп (индивидуальные тени, K с выводом ниже) + ссылки на КЛАСТЕРЫ
// хвоста — лампы бакета слиты в одну виртуальную С ОДНИМ теневым маршем
// (решение владельца 2026-08-24, отмена старого «без марша»: в интерьере
// хвост — перекрытые соседи, без тени они светили сквозь стены — репорт
// владельца с прожига). Это НЕ кластеризация неона:
// та — идентичность материи через связность атомов; эта — рендерное слияние
// далёких точечных ИСТОЧНИКОВ в один. Бюджеты лучей умирают естественно:
// список короток по построению, пиксель маршит каждый честный элемент.
//
// Бакет = 8 м = 2×2×2 клетки светосетки (ошибка позиции ≤ полудиагонали
// ≈ 6.9 м тонет в аттенюации хвостовых ламп — они дальние/слабые по
// построению). Слот кластера СТАБИЛЕН = kClusterSlotBase + индекс бакета:
// верхний регион таблицы ламп, статики/динамики [0..N) не дотягиваются.
// Размер бакета — ИЗ ЗАМЕРА КРИВОЙ 2026-08-24 (packed max худшей клетки,
// этаж 0 / этаж 5 при исходных 30 / 49):
//   бакет  8 м: 30 / 40 — хвост размазан, сжатия НЕТ (предпосылка старого
//                плана «8 м хватит» опровергнута данными);
//   бакет 16 м: 26 / 28 — вдвое на плотном, ничего на падике;
//   бакет 32 м: 17 / 18 — единственный размер, ломающий worst case.
// Цена 32 м: свет хвоста приходит от якоря (сильнейшего члена бакета), член
// может отстоять до ~полудиагонали бакета — искажение направления касается
// ТОЛЬКО хвоста за топ-K (доминанты честные). Если глаза владельца поймают
// «свет не оттуда» — возврат к 16 м одной константой с перезамером.
inline constexpr int kClusterGridDim = kLightVisDim / 8; // 8: бакет 32 м
inline constexpr std::uint32_t kClusterCount =
    static_cast<std::uint32_t>(kClusterGridDim) * kClusterGridDim *
    kClusterGridDim; // 32768
// = gpu::kRootLights (131072) − kClusterCount; шов game<->render держит
// static_assert в main.cpp (game рендерных капов не видит).
inline constexpr std::uint32_t kClusterSlotBase = 131072u - kClusterCount; // замер

// Топ-K честных ламп клетки. ВЫВОД: пиксель маршит каждую честную лампу
// списка; 8 = двойной запас над типичной плотностью БЛИЖНИХ доминирующих
// источников (4 ближе прочих при среднем 10.6 достижимых) — хвост за K по
// построению дальний/слабый и уходит в кластер. Константа с выводом, не
// ручка; проверяется глазами владельца (light-cluster.md §4).
inline constexpr std::uint32_t kHonestLampsPerCell = 8;

// Кластер бакета: якорь — позиция СИЛЬНЕЙШЕГО члена (кластер маршится;
// центроид ламп по разные стороны стены лёг бы внутрь стены и марш гасил бы
// кластер навсегда), радиус-охват (дистанция от якоря до члена + его радиус —
// консервативно), цвет — взвешенное среднее (вес radiusM²). members — CSR по
// memberSlots (слоты ламп; кадр суммирует их интенсивности в слот кластера).
struct LightVisCluster {
    std::uint32_t bucket = 0; // индекс бакета = слот − kClusterSlotBase
    vec3 pos{0.0f, 0.0f, 0.0f};
    float radiusM = 0.0f;
    vec3 color{1.0f, 1.0f, 1.0f};
...
struct LightVisBake {
    ...
    // Кластеры хвоста этого бейка (только непустые бакеты, по возрастанию
    // bucket) + CSR их членов (слоты ламп). Кадр суммирует интенсивности
    // членов в слот кластера (kClusterSlotBase + bucket) — мерцание кластера
    // усредняется, обесточка района гасит его, надгробие члена вычитается.
    ...
    // Упакованный МАКСИМУМ — цель кластеризации: худшая клетка (зал в 30
    // ламп) обязана стоить ~K+ссылки, а не 30 маршей.
    std::uint32_t packedMaxPerCell = 0;
    float bakeMs = 0.0f;

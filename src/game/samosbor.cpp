...
#include "game/combat.h"    // apply_damage, DamageChannel
#include "game/door.h"      // DoorSet, Door, DoorState
#include "game/embody.h"    // NpcRef, NpcId
...
// ---------------------------------------------------------------------------
// Continuous Active-phase environmental pressure
// ---------------------------------------------------------------------------
...
void samosbor_environmental_step(Registry& reg, NpcPool& pool,
    ...
    const SamosborVariant variant =
        static_cast<SamosborVariant>(sam.variant);

    // Pre-build a flat list of shut+locked hermetic doors on this floor for
    // O(n*m) shelter check where n = NPC count, m = hermetic door count.
    // In a typical residential floor m <= 20, so this is fast.
    struct HermeticCell { std::int32_t cx, cy, cz; };
    // stack-allocate up to 64 hermetic doors; beyond that, fall back to a
    // small heap vector — still bounded by the door count per floor (<= 2048).
    ...
    // Iterate every embodied entity on the active layer.
    ...
    for (auto e : view) {
        ...
        // Entity cell coordinates (toroidal 128^3 macro grid).
        // Replicate the exact pattern from main.cpp sealed block.
        const int pcx = ((static_cast<int>(std::floor(tr.pos.x / kCellSize)) %
                          kMacroDim) + kMacroDim) % kMacroDim;
        const int pcy = ((static_cast<int>(std::floor(tr.pos.y / kCellSize)) %
                          kMacroDim) + kMacroDim) % kMacroDim;
        const int pcz = ((static_cast<int>(std::floor(tr.pos.z / kCellSize)) %
                          kMacroDim) + kMacroDim) % kMacroDim;

        // Shelter check: within 4 cells (dx²+dy²+dz² ≤ 16) of a sealed
        // hermetic door — exact same predicate as main.cpp:3119-3129.
        ...
        switch (variant) {
            case SamosborVariant::Meat:
                // Meat fog accelerates dehydration (biological dissolution).
                n.water = std::max(0.0f, n.water - 0.1f * dt);
                break;
            case SamosborVariant::Electric:
                // Ionised air causes fatigue — sleep drains faster.
                n.sleep = std::max(0.0f, n.sleep - 0.1f * dt);
                break;
            case SamosborVariant::Veretar:
                // Veretar: direct corrosive dissolution, extra 0.3 HP/s.
                n.hpDebt += 0.3f * dt;
                break;
            default:
                break;
        ...
        // Spill whole HP debt through apply_damage so the Dead tag and
        // finalize_deaths contract are respected. kAttritionChannel bypasses
        // armour (same contract as starvation / dehydration).
        if (n.hpDebt >= 1.0f) {
            const auto whole = static_cast<std::int16_t>(n.hpDebt);
            n.hpDebt -= static_cast<float>(whole);
            apply_damage(reg, pool, e, whole, kAttritionChannel,
                               entt::null);
        }
    }

    // suppress -Wunused-variable when stack path never overflows
    (void)cellsPtr;
}

...
// СМЕРТЬ СТРАНИЦЫ — парная materialize_sub_page (CANON S16.9, марковское
// представление). Инвариант: страница существует ⟺ клетка неоднородна, и
// однородная выражается своим CellType целиком. Маска — производная фазы,
// поэтому однородная страница всегда имеет маску пустую либо полную, и свёртка
// с записью типа не теряет ни одного атома (вывод — S16.9).
//
// Любой путь, ЗАКОНЧИВШИЙ писать в клетку, обязан позвать settle: иначе
// представление копит историю, набор страниц растёт монотонно за сессию и
// уезжает в файл этажа навсегда (замер 2026-09-23: цена входа на этаж линейна
// по числу страниц). Рождение и смерть — здесь, в одном месте; звать их врозь
// по месту вызова = тот самый дефект.
//
// НЕ звать из обратного шва покадрово: клетка может опустеть и наполниться
// внутри одного кадра (до 8 подтиков), а снятая страница блокирует вход материи
// до ленивой материализации следующего кадра — физика начнёт зависеть от кадра
// (закон 2026-08-27, пин test_cadence_equivalence). Законные моменты — рождение
// и смерть этажа, когда автомат не бежит, и путь писателя, который сам сводит
// клетку. Честная смерть фронтирных страниц — выделение на GPU (инкремент 5).
//
// Возвращает true, если страница схлопнулась.
...
// ПРИБОР §77: сколько раз малый суд упёрся в бюджет и отдал дело большому.
// Гейт доказывает им, что повторные сиды ОДНОГО компонента не платят свои
// 512 узлов заново: при сносе здания число отсечек обязано считать РАЗНЫЕ
// компоненты, а не сиды (замер владельца — 82 944 отсечки за сессию, 595 мс
// на один карв из 232 клеток, полезной работы ноль).
std::uint32_t detach_cap_events();

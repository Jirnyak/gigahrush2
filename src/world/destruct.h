...
// шве по клеткам изменённых масок; судит ОПОРНЫЕ компоненты клетки
// (S20.5: атом подвижного материала опору не передаёт — cell_partition
// строится по load_bearing_words) и конвертирует отвязанное в рыхлых
// двойников. Возвращает число конвертированных атомов.
std::int32_t detach_judge_cells(World& w, const std::uint32_t* cells,
                                std::size_t n, CarveScratch& scratch,
                                CarveResult& out);

// Сид судьи на шве автомата: клетка + XOR-слова изменённых битов маски
// (старое ^ новое, накопленные за окно тишины). Гигиена S20.5 «сеять от
// изменения»: судятся только опорные компоненты, соседние с реально
// изменёнными атомами, а не все компоненты клетки — ход автомата меняет
// только подвижные биты, и их опорные соседи исчерпывают кандидатов.
struct JudgeSeed {
    std::uint32_t cell;
    std::uint64_t changed[8];
};

...
TWO INPUTS, and the split is the whole design decision:
  ...
  * data/items.csv columns 27..36 are the RECIPES. One recipe per item id, its
    cost vector being the item's own eight authored craft_* counts, its station
    ...
    `tierForComponents` over items.csv reproduced craft_tier for every row at
    export time). Re-typing those 3,536 integers into a second CSV would make
    "CSV edited, generator not re-run" an *authorable* failure rather than merely
    a possible one, so there is no second copy.
...
# The eight axes, in the order kraft.md calls "an API, a save and a UI contract".
# Reordering this list silently re-prices all 442 items and invalidates every save.
# The ninth axis was merged into craft_electronics on 2026-08-14 ([craft.h] "The
# eight axes"); read_items() below refuses a craft_* column this list does not
# name, so a resurrected ghost column dies here instead of being silently ignored
# by the DictReader.
MATERIAL_COLS = [
    "craft_mechanics", "craft_electronics", "craft_consumables", "craft_bio",
    "craft_chemical", "craft_metal", "craft_psimatter", "craft_metamatter",
]
...
def read_items():
    with open(ITEMS_PATH, encoding="utf-8", newline="") as fh:
        ...
        rows = list(reader)
        # A craft_* column this generator does not know is either a typo or a
        # dead axis coming back from the grave. DictReader would silently ignore
        # it and every one of its authored numbers would reach nothing — the
        # exact "authored and paid for, no reader" failure [craft.h] opens with.
        known = set(MATERIAL_COLS) | {"craft_station", "craft_tier"}
        ghosts = [c for c in (reader.fieldnames or [])
                  if c.startswith("craft_") and c not in known]
        if ghosts:
            die("items.csv carries craft column(s) nothing reads: %s — the axes "
                "are MATERIAL_COLS here and CraftMaterial in craft.h, one fact in "
                "two places" % ", ".join(ghosts))
...
def main():
    ...
    for i, r in enumerate(items):
        ...
        if total == 0:
            ...
            zero_rows += 1
            die("row %d (%s): all eight craft_* columns are 0. A zero composition "

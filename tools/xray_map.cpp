...
//   7. tower:  Global 100-floor megastructure cross-section, vertical biomes & elevator shafts
...
struct WorldContext {
    ...
    void init() {
        ...
        for (int f = kMinFloor; f <= kMaxFloor; ++f) {
            ...
            if (!def) continue;
            std::uint32_t fseed = compute_sector_seed(streamer.world_seed(), f);
    ...
    bool load_floor(int floorNum) {
        if (registry.module_at(floorNum) == kInvalidModule) {
            ...
            std::uint32_t fseed = compute_sector_seed(streamer.world_seed(), floorNum);
...
void draw_hud_header(Image& img, int floorNum, const char* modeName,
    ...
    char subtitle[256];
    VerticalBiome biome = floor_vertical_biome(floorNum);
    std::snprintf(subtitle, sizeof(subtitle),
                  "SECTOR: 1024m x 1024m (512x512) | BIOME: %s | POPULATION: %u",
                  vertical_biome_name(biome), popCount);
...
void render_mode_struct(Image& img, WorldContext& ctx, const ToolOptions& opt) {
    ...
    for (std::uint32_t py = 0; py < img.height; ++py) {
        int cy = static_cast<int>((static_cast<std::uint64_t>(py) * kMacroDimY) / img.height);
        for (std::uint32_t px = 0; px < img.width; ++px) {
            int cx = static_cast<int>((static_cast<std::uint64_t>(px) * kMacroDimX) / img.width);

            if (opt.sliceZ >= 0 && opt.sliceZ < kMacroDimZ) {
                // Exact 2D Z-slice
    ...
    for (int iy = 0; iy < kLatticeDim; ++iy) {
        for (int ix = 0; ix < kLatticeDim; ++ix) {
            ...
            int px = static_cast<int>((static_cast<std::int64_t>(hx) * img.width) / kMacroDimX);
            int py = static_cast<int>((static_cast<std::int64_t>(hy) * img.height) / kMacroDimY);
            int rLobby = static_cast<int>((static_cast<std::int64_t>(kFastLobbyR) * img.width) / kMacroDimX);
            int rShaft = static_cast<int>((static_cast<std::int64_t>(kFastShaftR) * img.width) / kMacroDimX);
    ...
    for (const Door& d : ctx.doors.doors) {
        int px = static_cast<int>((static_cast<std::int64_t>(d.cx) * img.width) / kMacroDimX);
        int py = static_cast<int>((static_cast<std::int64_t>(d.cy) * img.height) / kMacroDimY);
        int len = static_cast<int>((2 * img.width) / kMacroDimX);
        if (len < 2) len = 2;
        if (d.axis == 0) {
    ...
    if (opt.pop) {
        ...
        for (NpcId id : crowd) {
            ...
            int px = static_cast<int>((static_cast<std::int64_t>(cx) * img.width) / kMacroDimX);
            int py = static_cast<int>((static_cast<std::int64_t>(cy) * img.height) / kMacroDimY);
...
void render_mode_pop(Image& img, WorldContext& ctx, const ToolOptions& opt) {
    ...
    std::vector<float> density(static_cast<std::size_t>(kMacroDimX) * kMacroDimY, 0.0f);
    const auto& crowd = ctx.pool.floor_bucket(static_cast<std::int16_t>(ctx.loadedFloor));
    ...
    for (NpcId id : crowd) {
        ...
        for (int dy = -kRadius; dy <= kRadius; ++dy) {
            int cy = wrapi(ncy + dy, kMacroDimY);
            for (int dx = -kRadius; dx <= kRadius; ++dx) {
                int cx = wrapi(ncx + dx, kMacroDimX);
                float dist2 = static_cast<float>(dx * dx + dy * dy);
                if (dist2 <= kRadius * kRadius) {
                    float w = std::exp(-dist2 / (2.0f * 4.0f * 4.0f));
                    density[static_cast<std::size_t>(cy) * kMacroDimX + cx] += w;
                }
            }
        }
    }
    ...
    for (std::uint32_t py = 0; py < img.height; ++py) {
        ...
        for (std::uint32_t px = 0; px < img.width; ++px) {
            int cx = static_cast<int>((static_cast<std::uint64_t>(px) * kMacroDimX) / img.width);
            float norm = density[static_cast<std::size_t>(cy) * kMacroDimX + cx] / maxD;
            if (norm > 0.05f) {
    ...
    for (NpcId id : crowd) {
        ...
        int ncx = ctx.pool.cx(id);
        int ncy = ctx.pool.cy(id);
        int px = static_cast<int>((static_cast<std::int64_t>(ncx) * img.width) / kMacroDimX);
        int py = static_cast<int>((static_cast<std::int64_t>(ncy) * img.height) / kMacroDimY);
...
void render_mode_nav(Image& img, WorldContext& ctx, const ToolOptions& opt) {
    ...
    std::vector<std::uint16_t> distField(static_cast<std::size_t>(kMacroDimX) * kMacroDimY, 0xFFFFu);
    std::vector<int> qx, qy;
    qx.reserve(kMacroDimX * kMacroDimY);
    qy.reserve(kMacroDimX * kMacroDimY);
    ...
    for (int iy = 0; iy < kLatticeDim; ++iy) {
        for (int ix = 0; ix < kLatticeDim; ++ix) {
            int hx = lattice_coord(ix);
            int hy = lattice_coord(iy);
            std::size_t idx = static_cast<std::size_t>(hy) * kMacroDimX + hx;
            distField[idx] = 0;
            qx.push_back(hx);
            qy.push_back(hy);
        }
    ...
    while (head < qx.size()) {
        int cx = qx[head];
        int cy = qy[head];
        std::uint16_t curD = distField[static_cast<std::size_t>(cy) * kMacroDimX + cx];
        ++head;

        for (int d = 0; d < 4; ++d) {
            int nx = wrapi(cx + kDirs[d][0], kMacroDimX);
            int ny = wrapi(cy + kDirs[d][1], kMacroDimY);
            std::size_t nidx = static_cast<std::size_t>(ny) * kMacroDimX + nx;
    ...
    for (std::uint32_t py = 0; py < img.height; ++py) {
        ...
        for (std::uint32_t px = 0; px < img.width; ++px) {
            int cx = static_cast<int>((static_cast<std::uint64_t>(px) * kMacroDimX) / img.width);
            std::uint16_t d = distField[static_cast<std::size_t>(cy) * kMacroDimX + cx];
    ...
    for (std::uint32_t py = kStep / 2; py < img.height; py += kStep) {
        ...
        for (std::uint32_t px = kStep / 2; px < img.width; px += kStep) {
            ...
            for (int d = 0; d < 4; ++d) {
                int nx = wrapi(cx + kDirs[d][0], kMacroDimX);
                int ny = wrapi(cy + kDirs[d][1], kMacroDimY);
                std::uint16_t nd = distField[static_cast<std::size_t>(ny) * kMacroDimX + nx];
                if (nd < minD) { minD = nd; bestNx = nx; bestNy = ny; }
            }

            int dx = wrap_delta(cx, bestNx, kMacroDimX);
            int dy = wrap_delta(cy, bestNy, kMacroDimY);
            int len = 8;
...
void render_mode_danger(Image& img, WorldContext& ctx, const ToolOptions& opt) {
    ...
    // 3. Perimeter Radiation (>400m / >200 cells radius from center 256, 256)
    constexpr float kCenterCell = 256.0f;
    for (std::uint32_t py = 0; py < img.height; ++py) {
        float cy = (static_cast<float>(py) * kMacroDimY) / img.height;
        for (std::uint32_t px = 0; px < img.width; ++px) {
            float cx = (static_cast<float>(px) * kMacroDimX) / img.width;
            float r = std::hypot(cx - kCenterCell, cy - kCenterCell);

            if (r > static_cast<float>(kFuzzyBoundaryRadiusCells)) {
                float t = (r - kFuzzyBoundaryRadiusCells) / (256.0f - kFuzzyBoundaryRadiusCells);
                t = std::clamp(t, 0.0f, 1.0f);

                // Radiation glow: Yellow -> Red -> Radioactive Purple
                std::uint8_t rr = static_cast<std::uint8_t>(255);
                std::uint8_t rg = static_cast<std::uint8_t>(234 * (1.0f - t));
                std::uint8_t rb = static_cast<std::uint8_t>(249 * t);
                img.blend_pixel(px, py, rr, rg, rb, t * 0.75f);
            }
        }
    }

    // 4. Perimeter 400m warning circle
    int centerX = img.width / 2;
    int centerY = img.height / 2;
    int radiusPx = static_cast<int>((static_cast<std::int64_t>(kFuzzyBoundaryRadiusCells) * img.width) / kMacroDimX);
    img.draw_circle(centerX, centerY, radiusPx, 255, 23, 68, 0.9f);
    img.draw_circle(centerX, centerY, radiusPx + 2, 255, 234, 0, 0.7f);
    ...
    char radBuf[128];
    std::snprintf(radBuf, sizeof(radBuf),
                  "Perimeter Radiation: 1500 - 5000 uSv/h (>400m)");
    img.draw_string(legX + 8, legY + 24, radBuf, 255, 234, 0, 1);

    char samBuf[128];
    std::snprintf(samBuf, sizeof(samBuf),
                  "Samosbor Duty Cycle: %.1f%% (|z|=%d gradient)",
                  duty * 100.0f, std::abs(ctx.loadedFloor));
    img.draw_string(legX + 8, legY + 40, radBuf, 213, 0, 249, 1);

    img.draw_string(legX + 8, legY + 56, "Veretar Toxicity: High (Airlocks Sealed)", 255, 23, 68, 1);

    draw_hud_header(img, ctx.loadedFloor, "DANGER & PERIMETER HAZARD MAP",
                    ctx.catalog.resolve(ctx.loadedFloor).kind,
                    static_cast<std::uint32_t>(ctx.pool.floor_bucket(static_cast<std::int16_t>(ctx.loadedFloor)).size()));
}
...
void render_mode_props(Image& img, WorldContext& ctx, const ToolOptions& opt) {
    ...
    for (auto e : ctx.ecs.view<const Transform, const Interactable>()) {
        ...
        int px = static_cast<int>((tr.pos.x / kWorldExtentX) * img.width);
        int py = static_cast<int>((tr.pos.y / kWorldExtentY) * img.height);
        ...
        switch (ia.kind) {
            ...
            case InteractKind::Workbench:
                // Wood Crafting Bench
                img.fill_rect(px - 4, py - 4, 9, 9, 141, 110, 99, 0.9f);
                break;
            case InteractKind::KitchenStation:
                // Flame Stove
                img.fill_rect(px - 3, py - 3, 7, 7, 230, 74, 25, 0.9f);
                break;
            case InteractKind::MedicalStation:
                // Medical Cross
                img.fill_circle(px, py, 4, 0, 176, 255, 0.9f);
                break;
    ...
    draw_prop_key(legX + 8, legY + 38, "Workbench", 141, 110, 99);
    draw_prop_key(legX + 160, legY + 38, "Medical Bay", 0, 176, 255);
    draw_prop_key(legX + 8, legY + 54, "Light Fixture", 255, 255, 0);
    draw_prop_key(legX + 160, legY + 54, "Kitchen Station", 230, 74, 25);
...
// ---------------------------------------------------------------------------
// Mode 7: Global Tower Elevation & Cross-Section
// ---------------------------------------------------------------------------
void render_mode_tower(Image& img, WorldContext& ctx, const ToolOptions& opt) {
    img.init(img.width, img.height, 3, 7, 10);

    // Title
    img.fill_rect(0, 0, img.width, 36, 14, 35, 51, 0.9f);
    img.draw_line(0, 36, img.width - 1, 36, 92, 214, 255);
    img.draw_string(14, 12, "GIGAHRUSH 2 // MEGASTRUCTURE ELEVATION PROFILE // 100 FLOORS [-50 .. +50]",
                    92, 214, 255, 1);

    // Elevation bounds: +50 to -50
    constexpr int kTopFloor = 50;
    constexpr int kBotFloor = -50;
    constexpr int kFloorSpan = kTopFloor - kBotFloor + 1; // 101

    int marginL = 80;
    int marginR = static_cast<int>(img.width) - 220;
    int marginT = 50;
    int marginB = static_cast<int>(img.height) - 30;
    int drawH = marginB - marginT;

    // Draw 5 Biome Background Bands
    auto draw_biome_band = [&](int fTop, int fBot, const char* name,
                               std::uint8_t r, std::uint8_t g, std::uint8_t b) {
        int y0 = marginT + (kTopFloor - fTop) * drawH / kFloorSpan;
        int y1 = marginT + (kTopFloor - fBot + 1) * drawH / kFloorSpan;
        img.fill_rect(marginL, y0, marginR - marginL, y1 - y0, r, g, b, 0.25f);
        img.draw_rect(marginL, y0, marginR - marginL, y1 - y0, r, g, b);
        img.draw_string(marginL + 12, y0 + 6, name, r, g, b, 1);
    };

    draw_biome_band(50, 25, "UPPER CLEAN (RESEARCH & HIGH-SEC) [+50..+25]", 0, 229, 255);
    draw_biome_band(24, 1, "RESIDENTIAL (BLOCK HOUSING & MARKETS) [+24..+1]", 141, 110, 99);
    draw_biome_band(0, 0, "CENTRAL HUB ATRIUM [0]", 255, 179, 0);
    draw_biome_band(-1, -25, "INDUSTRIAL (HEAVY PIPELINES & WORKSHOPS) [-1..-25]", 216, 67, 21);
    draw_biome_band(-26, -50, "DEEP REACTOR & CATACOMBS [-26..-50]", 123, 31, 162);

    // Draw 16 Vertical Elevator Shaft Columns (Amber #FFB300)
    for (int ix = 0; ix < kLatticeDim; ++ix) {
        int hx = lattice_coord(ix);
        int sx = marginL + (hx * (marginR - marginL)) / kMacroDimX;
        img.draw_line(sx, marginT, sx, marginB, 255, 179, 0, 0.7f);
        img.draw_line(sx + 1, marginT, sx + 1, marginB, 255, 179, 0, 0.7f);
    }

    // Draw Floor ticks & Highlight Active Floor
    for (int f = kTopFloor; f >= kBotFloor; --f) {
        int fy = marginT + (kTopFloor - f) * drawH / kFloorSpan;
        if (f % 10 == 0 || f == 0) {
            img.draw_line(marginL - 8, fy, marginL, fy, 176, 190, 197);
            char fBuf[16];
            std::snprintf(fBuf, sizeof(fBuf), "%+d", f);
            img.draw_string(marginL - 40, fy - 3, fBuf, 176, 190, 197, 1);
        }

        // Highlight selected floor
        if (f == opt.floor) {
            img.draw_line(marginL - 12, fy, marginR + 12, fy, 255, 234, 0);
            img.fill_rect(marginL, fy - 2, marginR - marginL, 5, 255, 234, 0, 0.5f);
            img.draw_string(marginL - 70, fy - 3, ">> YOU", 255, 234, 0, 1);
        }
    }

    // Right Side: Demographics & Hazard Chart
    int statX = marginR + 20;
    int statW = static_cast<int>(img.width) - statX - 14;
    img.fill_rect(statX, marginT, statW, drawH, 14, 35, 51, 0.7f);
    img.draw_rect(statX, marginT, statW, drawH, 44, 122, 158);
    img.draw_string(statX + 8, marginT + 8, "TOWER METRICS:", 92, 214, 255, 1);

    // Plot Samosbor Duty Cycle Curve
    img.draw_string(statX + 8, marginT + 26, "Samosbor Hazard Curve:", 255, 23, 68, 1);
    for (int f = kTopFloor; f >= kBotFloor; --f) {
        int fy = marginT + (kTopFloor - f) * drawH / kFloorSpan;
        float d = samosbor_duty01(f);
        int barW = static_cast<int>(d * (statW - 30));
        img.draw_line(statX + 14, fy, statX + 14 + barW, fy, 255, 23, 68, 0.8f);
    }
}
...
int main(int argc, char** argv) {
    ...
    auto render_and_save = [&](const std::string& mode, const std::string& outPath) {
        ...
        else if (mode == "tower") render_mode_tower(img, ctx, opt);
    ...
    if (opt.allModes) {
        ...
        render_and_save("tower",  base + "_tower.png");
    } else {
